 ...
 */
module.exports.signRequest = function(auth, method, path, inBody, qs, contentType) {
  const timestamp = Date.now() / 1000;
  let body = '';
  if (inBody && contentType.startsWith('multipart')) {
    // body = inBody.getBuffer().toString('utf8');
    body = inBody
  } else {
    if (inBody) {
