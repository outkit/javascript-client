...
Client.prototype.getMessage = function(messageId, opts, callback) {
  ...
  return self._performRequest(reqOpts, null, callback);
};
...
Client.prototype.createMessage = function(message, opts, callback) {
  ...
  return self._performRequest(reqOpts, rawMessage, callback);
};

Client.prototype._performRequest = function(reqOpts, rawMessage, callback) {
  var self = this;
  ...
  var sig = self._getSignature(reqOpts, rawMessage);
...
Client.prototype._getSignature = function(reqOpts, rawMessage) {
  var self = this;
  var auth = {
    key: this.key,
    secret: this.secret,
  ...
  var sig = signRequest(auth, reqOpts.method, self._makeRelativeURI(reqOpts.url), rawMessage, reqOpts.qs, reqOpts.headers['content-type']);
  return {
